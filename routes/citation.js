...
/**
 * This will define the routes to be preformed on the  citation model
 *
 * path = /
 * @func GET
...
router.route('/')
  .get(async (req, res) => {
    ...
    let results;
    try {
      results = await citation.findAll({limit, where: queryOptions});
    } catch (err) {
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      writeResult = await citation.build(options).save();
    } catch (err) {
...
/**
 * This will find one instanse of the  citation model with matching prymaryKey
...
async function findOne(req, res) {
 ...
 try {
   ...
   const selection = await citation.findById(id);
   if (selection === null) {
