...
/**
 * This will define the routes to be preformed on the  chemical model
 *
 * path = /
 * @func GET
...
router.route('/')
  .get(async (req, res) => {
    ...
    let results;
    try {
      results = await chemical.findAll({limit, where: queryOptions});
    } catch (err) {
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      writeResult = await chemical.build(options).save();
    } catch (err) {
    ...
    res.status(201)
      .location(`/chemical/${writeResult.dataValues.id}`)
  ...
  router.route('/:id')
    ...
    .post(async (req, res) => {
      try {
        ...
        const resource = await chemical.findById(id);
        if (resource === null) {
    ...
    .put(async (req, res) => {
      try {
        ...
        let resource = await chemical.findById(id);
        if (resource === null) {
          res.status(404).json(new jsonResponse('resource not found'));
          return;
        }
    ...
    .patch(async (req, res) => {
      ...
      try {
        let instance = await chemical.findById(id);
        if (instance === null) {
          res.status(404).json(new jsonResponse('resource not found'));
          return;
        }
    ...
    .delete(async (req, res) => {
      ...
      try {
        ...
        instance = await chemical.findById(id);
      } catch (err) {
        res.status(500)
...
/**
 * This will find one instanse of the  chemical model with matching prymaryKey
...
async function findOne(req, res) {
 ...
 try {
   ...
   const selection = await chemical.findById(id);
   if (selection === null) {
